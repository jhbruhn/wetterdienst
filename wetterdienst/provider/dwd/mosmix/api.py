...
class DwdMosmixValues(TimeseriesValues):
    ...
    def query(self) -> Generator[ValuesResult, None, None]:
        ...
        for df in self._collect_station_parameter():
            ...
            if not df.is_empty() and self.sr.start_date and self.sr.start_date != DwdForecastDate.LATEST:
                df = df.filter(
                    (pl.col(Columns.DATE.value) >= self.sr.start_date)
                    & (pl.col(Columns.DATE.value) <= self.sr.end_date)
                )
    ...
    def _collect_station_parameter(self) -> Generator[pl.DataFrame, None, None]:
        """
    ...
    def read_mosmix(self, date: Union[dt.datetime, DwdForecastDate]) -> Generator[pl.DataFrame, None, None]:
        """
    ...
    def _read_mosmix(self, date: Union[DwdForecastDate, dt.datetime]) -> Generator[pl.DataFrame, None, None]:
        """
    ...
    def read_mosmix_small(self, date: Union[DwdForecastDate, dt.datetime]) -> Generator[pl.DataFrame, None, None]:
        """
