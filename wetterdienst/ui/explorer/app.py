...
def render_navigation_stations(payload):
    ...
    return [
        {"label": name, "value": station_id} for name, station_id in stations_data.select("name", "station_id").rows()
    ]
...
def render_map(payload):
    ...
    features = [
        ...
        for station_id, latitude, longitude in stations_data.select("station_id", "latitude", "longitude").rows()
    ]
